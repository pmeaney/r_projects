...
length(unique(perZip$zipcode)) # 31 zipcodes present
print(nrow(perZip)/2) # but some don't have both crimes accounted for

# select out all zipcodes with robbery -- something like this:
# select zipcode, NIBRS_OffenseDesc
# where NIBRS_OffenseDesc = robbery

robberyPerZip <- filter(perZip, NIBRS_OffenseDesc == "Robbery")
...
mysetdiff(perZip$zipcode, robberyPerZip$zipcode) 

# select out all zipcodes with burglary
burglaryPerZip <- filter(perZip, NIBRS_OffenseDesc == "Burglary")
View(burglaryPerZip) # has 28 rows

# This shows the three zipcodes missing in burglaryPerZip$zipcode
mysetdiff(perZip$zipcode, burglaryPerZip$zipcode)
